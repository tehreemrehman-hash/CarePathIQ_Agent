...
# To make the app private, we check for a password.
# Default password is 'carepathiq' if not set in secrets.
def check_password():
    """Returns `True` if the user has the correct password."""
    
    # 1. Get password from secrets or use default
    # In Streamlit Cloud, set 'APP_PASSWORD' in the Secrets management.
    correct_password = st.secrets.get("APP_PASSWORD", "carepathiq")
    ...
    if "password_correct" not in st.session_state:
        ...
        st.text_input(
            "🔒 This application is private. Please enter the password:", 
            type="password", 
