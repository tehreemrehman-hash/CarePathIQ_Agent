...
def create_ppt_presentation(slides_data, flowchart_img=None):
    """Generates a PowerPoint with text and optional flowchart image."""
    try:
        prs = Presentation()
    except NameError:
        return None
    
    # Layouts: 0=Title, 1=Title+Content, 5=Title Only (good for custom images), 6=Blank
    title_layout = prs.slide_layouts[0]
    content_layout = prs.slide_layouts[1]
    
    # 1. Title Slide
    slide = prs.slides.add_slide(title_layout)
    slide.shapes.title.text = slides_data.get('title', 'Clinical Pathway Launch')
    if slide.placeholders[1]:
        slide.placeholders[1].text = f"Target Audience: {slides_data.get('audience', 'General')}\nGenerated by CarePathIQ"

    # 2. Loop through Content Slides
    for slide_info in slides_data.get('slides', []):
        # Determine layout. If it's the "Format" slide and we have an image, use a layout that accommodates it better or just standard.
        ...
        # 3. Insert Flowchart if this is the "Format" slide
        if "Format" in slide_info.get('title', '') and flowchart_img:
            # Add image to the right side or bottom, resizing logic
            try:
                # Add picture (left, top, width) - Adjust positioning as needed
                # Position: 5 inches from left, 2 inches from top, width 4.5 inches
                slide.shapes.add_picture(flowchart_img, Inches(5), Inches(2), width=Inches(4.5))
                
                # Adjust text box width so it doesn't overlap the image
