...
st.markdown("""
<style>
    /* 1. ALL BUTTONS (Standard, Primary, Secondary) -> Dark Brown */
    div.stButton > button {
...
with st.sidebar:
    ...
    <div style="
        ...
        font-size: 0.9em;">
        Current Phase: <br>
        <span style="font-size: 1.1em; color: #4E342E;">{current_phase}</span>
    </div>
    """, unsafe_allow_html=True)
...
def get_gemini_response(prompt, json_mode=False):
    ...
    try:
        ...
        # Relaxed safety settings for medical terminology
        safety_settings = [
...
if "Phase 1" in phase:
    ...
    with col1:
        ...
        # AUTO-RUN: Trigger AI when condition changes
        if cond_input and cond_input != st.session_state.suggestions.get('condition_ref'):
            ...
            with st.spinner(f"🤖 AI Agent auto-populating suggestions for '{cond_input}'..."):
                prompt = f"""
